...
const MenuOption = ({defaultOpen, subAccounts, sidebarOpt, sidebarLogo, details, user, id}: Props) => {
    ...
    return (
        <Sheet modal={false} {...openState}>
            ...
            <SheetContent showX={!defaultOpen} side="left" className={clsx("bg-background/80 backdrop-blur-xl fixed" +
                " top-0 border-r-[1px] p-6", {
                                        ...
                                        </CommandEmpty>
                                        {
                                            (user?.role === "AGENCY_OWNER" || user?.role === "AGENCY_ADMIN") && user?.Agency && (
                                                <CommandGroup heading="Agency">
                                                    <CommandItem
                                                        className="!bg-transparent my-2 text-primary border-[1px] border-border p-2 rounded-md hover:!bg-muted cursor-pointer transparent-all">
                                                        {defaultOpen ?
                                                            (
                                                                <Link href={`/agency/${user?.Agency?.id}`}
                                                                    ...
                                                                    <div className="flex flex-col flex-1">
                                                                        {user?.Agency?.name}
                                                                        <span
                                                                            className="text-muted-foreground">{user?.Agency?.address}
                                                                    </span>
                                                                    </div>
                                                                </Link>
                                                            )
                                                            ...
                                                            (
                                                                <Link href={`/subaccount/${subaccount?.id}`}
                                                                    ...
                                                                    <div className="flex flex-col flex-1">
                                                                        {subaccount?.name}
                                                                        <span
                                                                            className="text-muted-foreground">{subaccount?.address}
                                                                    </span>
                                                                    </div>
                                                                </Link>
                                                            )
                                            ...
                                            <Button className="w-full flex gap-2" onClick={() => {
                                                setOpen(
                                                    <CustomModal title="Create A Subaccount"
                                                                 subheading="You can switch between your account and the subaccount from the sidebar">
